...
> **Лучше ли ваш скилл, чем его отсутствие?** Каждый кейс прогоняется дважды - со скиллом и без него, на той же модели.
> Если модель справляется сама, скилл получает статус **OBSOLETE**, как бы хорошо он ни был написан.
> Большинство репозиториев со скиллами этого не проверяют.
...
**→ [skillemall.ai](https://skillemall.ai)** — публичный рейтинг на этом движке: 120 000+ скиллов из открытых
...
| **baseline** | Добавляет ли скилл хоть что-то по сравнению с той же моделью *без* скилла? | прирост ниже порога → статус **OBSOLETE** (роняет сборку с `--strict` или `skilleval ci`; по умолчанию код выхода 0) |
